...
def make_interactive_plot(proj_x1,
                          proj_y1,
                          proj_x2,
                          proj_y2,
                          extent,
    ...
    ax[0].scatter(proj_x1[:,0],                  # 输入数据的x轴坐标
                proj_x1[:,1],                    # 输入数据的y轴坐标
                c = proj_y1.argmax(axis=1),      # 色彩序列
                marker=',',                     # 图中点的形状：像素
                s=1,                            # maker size，决定图中点的大小
                cmap='Reds'
    )

    ax[0].scatter(proj_x2[:,0],                  # 输入数据的x轴坐标
                proj_x2[:,1],                    # 输入数据的y轴坐标
                c = proj_y2.argmax(axis=1),      # 色彩序列
                marker=',',                     # 图中点的形状：像素
                s=1,                            # maker size，决定图中点的大小
                cmap='PuBu'
    )
...
if __name__ == '__main__':
    ...
    make_interactive_plot(proj_m_x_train,
              m_y_train,
              proj_advm_x_train,
              advm_y_train,
              [zmin, zmax, zmin, zmax],
              plot_ent,         # plot_ent , plot_bald
              decoder,
              model,
    )

    make_starred_plot(proj_m_x_train,
                      m_y_train,
                      proj_advm_x_train,
                      advm_y_train,
                      [zmin, zmax, zmin, zmax],
                      plot_ent,
                      decoder,
              np.array([[-0.24, -1.05], [-0.25, -0.92], [-0.30, -0.97]])
    )
    print('done')              
    plt.savefig('Result_picture/Sample_AdvMNIST_ResNet34_MC_Ent2')
    plt.show()
