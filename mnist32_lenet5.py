...
x_train, y_train, x_test, y_test = U2.get_mnist_32()
...
model.add(Conv2D(6, kernel_size=(5, 5),                # 卷积层，Conv2D创建二维卷积层，filters: 整数，输出空间的维度 （即卷积中滤波器的数量）。
...
model.add(Conv2D(16, (5, 5),padding='SAME', activation='relu'))        # 卷积层，输出为[24,24,64]
...
model.add(Dense(120, activation='relu'))                # 全连接层，output = activation(dot(input, kernel)+bias)，此步输出为[128]
...
fname = U.gen_save_name('save/mnist32_lenet5_run.h5')
model.save(fname)
